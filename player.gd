...
func movement():
	...
	if Input.is_action_pressed("up") and position.y > 0: # move player up if below y=0
		position.y -= 10
	if Input.is_action_pressed("down") and position.y < screen_size.y: # move player down if above y=screenheight
		position.y += 10
